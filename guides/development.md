...
- `FLY_TEST_SETTINGS=/path/to/private.json bun run test:fly`: 明示的な実環境テストです。JSONには `kituneOrigin`・`dexOrigin`・`kituneApp`・`dexApp`・`smokeUser`・`smoke`（テストクライアントsecret）を指定します。事前にKituneに未使用の `deploy-test-*` ユーザー、Dexに `deployment-smoke` クライアント（callback `http://127.0.0.1:9876/callback`）を設定します。Kituneの既存 `personal-dex` クライアントを使い、TLS、Passkey、両段の同意、OIDC、グループ、各3回の復帰、両方休止からの更新、失効を確認します。`FLY_TEST_LIFECYCLE=stop`（既定）は完全停止、`FLY_TEST_LIFECYCLE=suspend` は休止を検証します。対象は各1 Machine限定です。終了時にテスト資格情報を失効させますが、本番設定への復帰と再デプロイは別途必要です。削除済みのユーザーIDは次の試験で再利用しません。結果は `test-results/fly-stop.json` または `test-results/fly-suspend.json` に保存します。
