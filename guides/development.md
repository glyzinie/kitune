...
- 同意画面はscopeと個別の `claims.userinfo` 要求を表示し、表示したscope・claimの集合を同意APIへ明示します。対応する標準claimは `src/consent.ts` にあり、Provider更新時には実際のUserInfoと照合します。署名付き `oauth_query` の照合と認可処理はProviderが行います。
