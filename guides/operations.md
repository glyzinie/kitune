...
初期値はshared CPU 1基・512MBです。低負荷時は `stop` で停止し、リクエストで起動します。DexのDiscovery・UserInfo・トークン更新もリクエストに含まれます。実環境では複数回の完全停止からの起動、OIDC経由の起動、ピークメモリ、OOM、タイムアウトを測ってからサイズを見直します。1台構成なのでデプロイ中・障害時には停止時間が発生します。
